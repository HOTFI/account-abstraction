...
contract HotfiAccount is BaseAccount, TokenCallbackHandler, UUPSUpgradeable, Initializable {
     ...
     */
    function initialize(address anOwner, bytes[] memory initOps) public virtual initializer {
        _initialize(anOwner, initOps);
    }

    function _initialize(address anOwner, bytes[] memory initOps) internal virtual {
        owner = anOwner;

        for (uint256 index = 0; index < initOps.length; ++index) {
            (address target, uint256 value, bytes memory data) = abi.decode(initOps[index], (address, uint256, bytes));
