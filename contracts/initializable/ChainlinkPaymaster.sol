...
contract ChainlinkPaymaster is BasePaymaster {
    ...
    function getTokenValueOfEth(uint256 ethOutput) internal view returns (uint256 tokenInput){
        uint lastPrice = uint256(_tokenToEthFeed.latestAnswer());
    ...
    internal view override returns (bytes memory context, uint256 validationData) {
        ...
        require(userOp.verificationGasLimit > COST_OF_POST, "ChainlinkPaymaster: gas too low for postOp");

        if (userOp.initCode.length != 0) {
            _validateConstructor(userOp);
            require(feeToken.balanceOf(userOp.sender) >= tokenPrefund, "ChainlinkPaymaster: no balance (pre-create)");
        } else {
            require(feeToken.balanceOf(userOp.sender) >= tokenPrefund, "ChainlinkPaymaster: no balance");
        }

        require(feeToken.allowance(userOp.sender, address(this)) >= tokenPrefund, "ChainlinkPaymaster: no allowance");
        uint256 gasPriceUserOp = userOp.gasPrice();
        return (abi.encode(userOp.sender, gasPriceUserOp, maxCost, tokenPrefund), 0);
    }
    ...
    function _validateConstructor(UserOperation calldata userOp) internal virtual view {
        ...
        require(factory == theFactory, "ChainlinkPaymaster: wrong account factory");
    }
    ...
    function _postOp(PostOpMode mode, bytes calldata context, uint256 actualGasCost) internal override {
        ...
        (address sender, uint256 gasPricePostOp, /*uint256 maxCost*/,uint256 tokenPrefund) = 
            abi.decode(context, (address, uint256, uint256, uint256));
        uint256 charge = getTokenValueOfEth(actualGasCost + COST_OF_POST * gasPricePostOp);
        
        require(charge <= tokenPrefund, "ChainlinkPaymaster: gas price too low");
